    ...
    def get_results(self, html):
        ...
        if len(soup.select('.card-no-result'))>0:
            ...
            for i in soup.select('div[action-type="feed_list_item"]'):
                ...
                print("#############################")
                weibotext = ""
                for text in i.find_all("p", "txt"):
                    if(len(text.getText().replace(" ", ""))>len(weibotext)):
                        weibotext = text.getText().replace(" ", "")  
                blog['微博内容'] = weibotext
##                print(blog['微博内容'])# 如果有特殊字符输出时，IDLE不支持会报错，并不是代码有bug
                print("#############################")
...
##                    html_page = html_page.translate(non_bmp_map)

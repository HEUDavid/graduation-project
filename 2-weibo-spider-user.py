...
session.cookies = cookielib.LWPCookieJar(filename='mycookie')
try:
...
##            html = requests.get(url, cookies=self.cookie).content
            html = session.get(url, allow_redirects=False).content
...
##            html = requests.get(weibo_link, cookies=self.cookie).content
            html = session.get(weibo_link, allow_redirects=False).content
            selector = etree.HTML(html)
...
##                html2 = requests.get(url2, cookies=self.cookie).content
                html2 = session.get(url2, allow_redirects=False).content
...
def main():
    try:
        ...
        user_id = 1578022273  # 可以改成任意合法的用户id（爬虫的微博id除外）
